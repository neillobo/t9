...
var wordsArray= _.map(words.split(" "), function (word) {
  return word.split("");
});
...
Node.prototype.getWordLocations = function(keys) {
  ...
  if (keys.length === 1) {
    ...
    var result = [];
    result.concat(this._wordLocations);
    for (var i = 0; i < 10; i++) {
      if (this._children[i] !== null) {
        result.concat(this._children[i].getWordLocations(keys));
      }
    }
    return result;
...
var buildTree = function(wordsList) {
  ...
  for (var i = 0; i < wordsList.length; i++ ) {
    head.addWord(wordsList[i], i);
  };


  head.retrieve = function (keys) {
    //convert the string to an array
    var roger = keys.split("");
    var test = keys.split("");
    console.log(typeof keys);

    //this will return an array of locations
    var resultsLocations = head.getWordLocations(test);
    debugger;
